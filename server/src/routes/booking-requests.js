...
router.get('/', async (req, res) => {
  try {
    ...
    let all = await db.findAll('booking_requests')
...
router.put('/:id/respond', async (req, res) => {
  try {
    ...
    if (response === 'yes') {
      await db.update('booking_requests', id, { status: 'confirmed', decided_at: new Date().toISOString() })
      await auditUpdate(req, 'booking_request', request.id, { status: request.status }, { status: 'confirmed' })
      res.json({ ok: true, message: 'Attendance confirmed' })
    } else if (response === 'no') {
      await db.update('booking_requests', id, { status: 'denied', decided_at: new Date().toISOString() })
      await auditUpdate(req, 'booking_request', request.id, { status: request.status }, { status: 'denied' })
...
router.post('/', async (req, res) => {
  try {
    ...
    const slot = await db.get('slots', parseInt(slot_id))
...
router.put('/:id/decide', requireRole('superadmin', 'admin'), async (req, res) => {
  try {
    ...
    const slot = await db.get('slots', request.slot_id)
    ...
    if (decision === 'approved') {
      if (request.kind === 'cancel') {
        if (slot) {
          ...
          if (slot.user_id && slot.status !== STATUS.PLAYER_CONFIRMED) {
            ...
            if (user) {
              ...
              await reverseBalance(user.id, sessionType)
            }
          }
        }
        if (request.booking_id) {
          const booking = await db.get('bookings', request.booking_id)
          if (booking) {
            // Check if all slots for this booking are cancelled or none left
            const remainingSlots = await db.findAll('slots', s => s.booking_id === booking.id && s.status !== STATUS.AVAILABLE && s.status !== STATUS.CANCELLED)
            if (remainingSlots.length === 0) {
              await db.update('bookings', booking.id, { status: 'cancelled' })
            }
            if (booking.user_id) {
              await notify(booking.user_id, 'request_approved', 'Cancellation Approved',
        ...
        if (slot && proposed_date && proposed_time) {
          ...
          const conflict = await db.find('slots', s => s.date === proposed_date && s.time === proposed_time && s.court === newCourt && s.id !== slot.id)
          if (conflict) {
            return res.status(409).json({ error: 'Target slot is already occupied' })
          }
          await db.update('slots', slot.id, { date: proposed_date, time: proposed_time, court: newCourt })
        }
        if (request.booking_id) {
          const booking = await db.get('bookings', request.booking_id)
          if (booking && booking.user_id) {
            await notify(booking.user_id, 'request_approved', 'Modification Approved',
              `Your slot has been moved to ${proposed_date} ${proposed_time}.`, '/schedule')
          }
        }
      } else if (request.kind === 'reschedule_offer') {
        if (slot && proposed_date && proposed_time) {
          const conflict = await db.find('slots', s => s.date === proposed_date && s.time === proposed_time && s.court === (proposed_court || slot.court) && s.id !== slot.id)
          if (conflict) {
            return res.status(409).json({ error: 'Proposed slot is already occupied' })
          }
        }
        if (request.player_id) {
          await notify(request.player_id, 'reschedule_offer', 'New Time Proposed',
      ...
      if (request.kind === 'cancel') {
        if (request.player_id) {
          await notify(request.player_id, 'request_denied', 'Cancellation Denied',
            `Your cancellation request for ${slot?.date} ${slot?.time} was denied.`, '/schedule')
        }
      } else {
        if (request.player_id) {
          await notify(request.player_id, 'request_denied', 'Request Denied',
    ...
    const updated = await db.update('booking_requests', id, {
      ...
      payload: { ...(typeof request.payload === 'string' ? JSON.parse(request.payload || '{}') : request.payload || {}), proposed_date, proposed_time, proposed_court },
    })
    await auditUpdate(req, 'booking_request', id, { status: request.status }, { status: decision, kind: request.kind, slot_id: request.slot_id })
    res.json(updated)
  } catch (err) {
